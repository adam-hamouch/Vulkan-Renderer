...
struct GpuMat4
{
    float data[16];
    GpuMat4() {}
    GpuMat4(const Mat4& m) { memcpy(data, m.data, sizeof(data)); }
};
...
namespace std {
    template<> struct hash<Vector2D> {
        size_t operator()(Vector2D const& v) const {
            return hash<float>()(v.x) ^ (hash<float>()(v.y) << 1);
        }
    };

    template<> struct hash<Vector3D> {
        size_t operator()(Vector3D const& v) const {
    ...
    template<> struct hash<Vertex> {
        size_t operator()(Vertex const& vertex) const {
            return ((hash<Vector3D>()(vertex.pos) ^
                    (hash<Vector3D>()(vertex.color) << 1)) >> 1) ^
                    (hash<Vector2D>()(vertex.texCoord) << 1);
        }
    };
}
    ...
    void createGraphicsPipeline() 
{
    ...
    void updateUniformBuffer(uint32_t currentImage) 
    {
        ...
        ubo.model = GpuMat4(Mat4::RotationZ(DegToRad(time * 90.0f)).Transpose());
        ubo.view = GpuMat4(camera->GetViewMatrix().Transpose());
        Mat4 proj = camera->GetProjectionMatrix();
        proj.data[5] *= -1.0f; 
        ubo.proj = GpuMat4(proj.Transpose());
    ...
    void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
    {
        ...
        vkCmdCopyBufferToImage(          // ← avant endSingleTimeCommands
            commandBuffer,
            buffer,
            image,
    ...
    if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT))
        throw std::runtime_error("le format de texture ne supporte pas le filtrage lineaire!");
